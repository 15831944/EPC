...
", tableName, row["ID"], insertSql, updateSql);
        ...
        private static Dictionary<string, SQLHelper> sqlHelperDic = new Dictionary<string, SQLHelper>();
        public static SQLHelper CreateSqlHelper(EnumConn conn)
        {
            string connName = conn.ToString();
            string key = string.Format("Conn_{0}", connName);
            string connString = System.Configuration.ConfigurationManager.ConnectionStrings[connName].ConnectionString;
            SQLHelper sqlHelper = null;
            if (sqlHelperDic.GetValue(key) != null)
                return sqlHelperDic[key] as SQLHelper;

            if (String.IsNullOrEmpty(connString))
                throw new Exception(string.Format("连接字符串不能为空值：{0}", connName));
            sqlHelper = new SQLHelper(connString);
            sqlHelperDic[key] = sqlHelper;
            return sqlHelper;
        }
