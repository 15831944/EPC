...
namespace Interface.Logic
{
    public static class HttpHelper
    {
        ...
        public static string Post( string requestUrl, string paramJsonStr)
        {
            var restClient = new RestSharp.RestClient();
            var request = new RestSharp.RestRequest(requestUrl, RestSharp.Method.POST);
            request.RequestFormat = RestSharp.DataFormat.Json;
            //request.AddBody(paramJsonStr);
            request.AddParameter("application/json", paramJsonStr, RestSharp.ParameterType.RequestBody);
        ...
        public static T Post<T>(string serverUrl, string requestUrl, object data = null) where T : class
        {
            ...
            var result = JsonHelper.ToObject<T>(content);
            return result;
        }
    }
}
