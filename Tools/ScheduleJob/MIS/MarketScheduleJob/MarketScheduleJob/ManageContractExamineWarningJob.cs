...
{
    [Description("未审批结束合同预警")]
    public class ManageContractExamineWarningJob : IJob
    {
        /// <summary>
        /// 提醒类型
        /// </summary>
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            //获取提醒配置信息（市场经营内配置页面）
            DataRow dr = common.GetAlarmConfigRow(MarketScheduleJob.Enum.AlarmTypeEnum.ManageContractExamineWarning.ToString());
            //无配置信息时不提醒
            if (dr == null)
                return;
            ...
            string where = string.Format(" Where CONVERT(varchar(100), CreateDate, 112) <'{0}' And ( FlowPhase!='End' Or FlowPhase is null) ", date);

            //获取需要提醒的合同信息
            string alarmSql = string.Format("SELECT * From  dbo.S_C_ManageContract {0}", where);
            DataTable dt = marketHelper.ExecuteDataTable(alarmSql);

            //获取扩展（表单）接收人字典
            Dictionary<string, string> dic = common.GetAlarmExendPerson("S_C_ManageContract", dr[ALARMEXENDPERSONIDFIELD].ToString(), dr[ALARMEXENDPERSONNAMEFIELD].ToString(), where);


            //指定接收人提醒
            StringBuilder alarmPersonContent = new StringBuilder(string.Format("自登记日起,{0}天内未审批结束的合同如下：</br>", addDays));
            if (dr[ALARMPERSONID] != null)
            {
                //指定接收人提醒客户
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    alarmPersonContent.AppendFormat("合同名称：{0},</br>合同编号：{1}；</br></br>", dt.Rows[i]["Name"], dt.Rows[i]["Code"]);
                }

                //提醒内容不为空时
                if (alarmPersonContent.ToString() != string.Format("自登记日起,{0}天内未审批结束的合同如下：</br>", addDays))
                {
                    //每个接收人发一条提醒
            ...
            if (dic != null)
            {
                foreach (var item in dic)
                {
                    ...
                    //指定接收人提醒客户
                    StringBuilder alarmExendPersonContent = new StringBuilder(string.Format("自登记日起,{0}天内未审批结束的合同如下：</br>", addDays));
                    foreach (var row in customerRows)
                        alarmExendPersonContent.AppendFormat("合同名称：{0},</br>合同编号：{1}；</br></br>", row["Name"], row["Code"]);

                    //提醒内容不为空时
                    if (alarmExendPersonContent.ToString() != string.Format("自登记日起,{0}天内未审批结束的合同如下：</br>", addDays))
