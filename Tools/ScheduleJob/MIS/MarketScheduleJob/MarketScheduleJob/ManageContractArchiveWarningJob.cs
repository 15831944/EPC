...
{
    [Description("未归档合同预警")]
    public class ManageContractArchiveWarningJob : IJob
    {
        /// <summary>
        /// 提醒类型
        /// </summary>
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            //获取提醒配置信息（市场经营内配置页面）
            DataRow dr = common.GetAlarmConfigRow(MarketScheduleJob.Enum.AlarmTypeEnum.ManageContractArchiveWarning.ToString());
            //无配置信息时不提醒
            if (dr == null)
                return;
            ...
            string where = string.Format(" Where CONVERT(varchar(100), CreateDate, 112) <'{0}'  And (IsContractArchive!='是' Or IsContractArchive is null )", date); ;
            ...
            //指定接收人提醒
            StringBuilder alarmPersonContent = new StringBuilder(string.Format("自登记日起,{0}天内未归档的合同如下：</br>", addDays));
            if (dr[ALARMPERSONID] != null)
            {
                ...
                if (alarmPersonContent.ToString() != string.Format("自登记日起,{0}天内未归档的合同如下：</br>", addDays))
                {
                    //每个接收人发一条提醒
            ...
            if (dic != null)
            {
                foreach (var item in dic)
                {
                    ...
                    //指定接收人提醒客户
                    StringBuilder alarmExendPersonContent = new StringBuilder(string.Format("自登记日起,{0}天内未归档的合同如下：</br>", addDays));
                    foreach (var row in customerRows)
                        alarmExendPersonContent.AppendFormat("合同名称：{0},</br>合同编号：{1}；</br></br>", row["Name"], row["Code"]);

                    if (alarmExendPersonContent.ToString() != string.Format("自登记日起,{0}天内未归档的合同如下：</br>", addDays))
