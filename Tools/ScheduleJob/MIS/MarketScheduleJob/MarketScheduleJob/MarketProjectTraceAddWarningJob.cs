...
namespace MarketScheduleJob
{
    ...
    public class MarketProjectTraceAddWarningJob : IJob
    {
        /// <summary>
        /// 提醒类型
        /// </summary>
        ...
        /// 提醒标题
        /// </summary>
        string ALARMTITLE = "前期项目跟踪信息填写预警";
        /// <summary>
        /// 指定提醒人
        /// </summary>
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            //获取提醒配置信息（市场经营内配置页面）
            DataRow dr = common.GetAlarmConfigRow(MarketScheduleJob.Enum.AlarmTypeEnum.MarketProjectTraceAddWarning.ToString());
            //无配置信息时不提醒
            if (dr == null)
                return;
            ...
            string where = string.Format(" Where CONVERT(varchar(100), CreateDate, 112) <'{0}' And ID NOT IN (SELECT Distinct MarketProjectID From dbo.S_P_MarketProjectTrack) ", date);

            //获取需要提醒的客户信息
            string alarmSql = string.Format("SELECT * From  dbo.S_P_MarketProject {0} ", where);
            DataTable dt = marketHelper.ExecuteDataTable(alarmSql);

            //获取扩展（表单）接收人字典
            Dictionary<string, string> dic = common.GetAlarmExendPerson("S_P_MarketProject", dr[ALARMEXENDPERSONIDFIELD].ToString(), dr[ALARMEXENDPERSONNAMEFIELD].ToString(), where);

            //指定接收人提醒
            StringBuilder alarmPersonContent = new StringBuilder(string.Format("自登记日起,{0}天内未查询到以下项目的跟踪信息：</br>", addDays));
            if (dr[ALARMPERSONID] != null)
            {
                //指定接收人提醒客户
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    alarmPersonContent.AppendFormat("项目名称：{0}，</br>项目编号：{1}；</br></br>", dt.Rows[i]["PrjName"], dt.Rows[i]["PrjCode"]);
                }

                //提醒内容不为空时
                if (alarmPersonContent.ToString() != string.Format("自登记日起,{0}天内未查询到以下项目的跟踪信息：</br>", addDays))
                {
                    //每个接收人发一条提醒
            ...
            if (dic != null)
            {
                ...
                {
                    DataRow[] marketProjectRows = dt.Select(common.GetAlarmExendPersonIDWhereFormat(dr[ALARMEXENDPERSONIDFIELD].ToString(), item.Key));

                    //指定接收人提醒客户
                    StringBuilder alarmExendPersonContent = new StringBuilder(string.Format("自登记日起,{0}天内未查询到以下项目的跟踪信息：</br>", addDays));
                    foreach (var row in marketProjectRows)
                        alarmExendPersonContent.AppendFormat("项目名称：{0}，</br>项目编号：{1}；</br></br>", row["PrjName"], row["PrjCode"]);

                    //提醒内容不为空时
                    if (alarmExendPersonContent.ToString() != string.Format("自登记日起,{0}天内未查询到以下项目的跟踪信息：</br>", addDays))
