...
public class SWFConvert
{
    ...
    public static bool JPG2SWF(string jpgPath, string swfPath)
    {
        string exe = AppDomain.CurrentDomain.BaseDirectory + "\\SWFTool\\jpeg2swf.exe";
        if (!System.IO.File.Exists(exe) || !System.IO.File.Exists(jpgPath) || System.IO.File.Exists(swfPath))
            return false;
        StringBuilder sb = new StringBuilder();
        sb.Append(" \"" + jpgPath + "\"");
        sb.Append(" -o \"" + swfPath + "\"");
    ...
    public static bool PNG2SWF(string pngPath, string swfPath)
    {
        string exe = AppDomain.CurrentDomain.BaseDirectory + "\\SWFTool\\png2swf.exe";
        if (!System.IO.File.Exists(exe) || !System.IO.File.Exists(pngPath) || System.IO.File.Exists(swfPath))
            return false;
        StringBuilder sb = new StringBuilder();
        sb.Append(" \"" + pngPath + "\"");
        sb.Append(" -o \"" + swfPath + "\"");
