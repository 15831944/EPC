...
{
    public partial class S_EP_ProductionSettleRevert : BaseEPModel
    {
        public void Push()
        {
            var applyDic = this.GetDataDicByID("S_EP_ProductionBatchSettleApply", this.GetValue("SettleID"));
            if (applyDic == null)
            {
...
where ID='{0}'", item));
            ...
            applyDic.UpdateDB(this.DB, "S_EP_ProductionBatchSettleApply", applyDic.GetValue("ID"));
