...
where S_I_UserDefaultProjectInfo.UserID='" + userID + "' order by S_I_UserDefaultProjectInfo.ID desc";
        ...
        public void SetUserProjectList(string userID, string projectInfoID)
        {
            ...
            var exist = list.FirstOrDefault(a => a.ProjectInfoID == projectInfoID);
            if (exist != null)
            {
                //已经存在，并且是最近打开的，不操作
        ...
        public string GetDefaultProjectID(string userID)
        {
            ...
            {
                var projectInfo = sqlHelper.ExecuteScalar("select ID from S_I_ProjectInfo with(nolock) where ID='" + defaultProject.ProjectInfoID + "'");
                if (projectInfo == null || projectInfo == DBNull.Value)
                {
