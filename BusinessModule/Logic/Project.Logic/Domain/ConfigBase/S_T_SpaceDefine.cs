...
namespace Project.Logic.Domain
{
        ...
        S_T_SpaceDefine
    {
        ...
        S_T_SpaceDefine _parent = null;
        /// <summary>
        /// 根节点定义
        /// </summary>
        [NotMapped]
        [JsonIgnore]
        public S_T_SpaceDefine Parent
        {
            get
            {
                if (_parent == null)
                {
                    if (this.S_T_ProjectMode == null)
                        throw new Formula.Exceptions.BusinessException("未能找管理模式对象，获取SpaceDefine根节点失败");
                    _parent = this.S_T_ProjectMode.S_T_SpaceDefine.FirstOrDefault(d => d.ID == this.ParentID);
                }
                return _parent;
            }
        }
