...
left join S_T_SpaceDefine  on S_T_SpaceAuth.SpaceID=S_T_SpaceDefine.ID where RoleCode in ('{0}') and Type='Root' and ModeID='{1}'";
        ...
        public List<S_T_SpaceDefine> GetSpaceDefineByProjectRoles(string defineRootID, List<S_W_OBSUser> roleList, string relateCode)
        {
            ...
            foreach (var item in list)
            {
                foreach (var role in roleList)
                {
                    ...
                    (relateCode == role.MajorValue || relateCode == "Project"));
                    if (spaceAuth != null)
                    {
        ...
        public void InitDeisgnInputTemplate(bool isBuild = false)
        {
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                if (!isBuild)
                    input = this.S_D_Input.FirstOrDefault(d => d.InfoName == name);
                if (input == null)
                {
                    input = new S_D_Input();
                    ...
                    input.CreateUserID = CurrentUserInfo.UserID;
                    input.CreateUser = CurrentUserInfo.UserName;
                ...
                input.ModifyUserID = CurrentUserInfo.UserID;
                input.ModifyUser = CurrentUserInfo.UserName;
            }
        }
