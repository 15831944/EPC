...
namespace Project.Logic.Domain
{
    public partial class S_W_WBS
    {
            ...
            bool ignoreValidateError = false)
        {
            ...
            else if (this.WBSType == WBSNodeType.Major.ToString())
                rbsUser.MajorValue = this.WBSValue;//专业节点还没有赋值MajorCode的情况
        ...
        public void Save(bool sychUser = true)
        {
            ...
            if (this.S_W_TaskWork.Count > 0)
            {
                foreach (var tw in this.S_W_TaskWork.ToList())
                {
                    tw.PlanStartDate = this.PlanStartDate;
                    tw.PlanEndDate = this.PlanEndDate;
                    tw.Name = this.Name;
                    if (!String.IsNullOrEmpty(this.Code))
                        tw.Code = this.Code;
                    tw.ModifyDate = DateTime.Now;
                    tw.ModifyUser = user.UserName;
                    tw.ModifyUserID = user.UserID;
                }
            }
