    ...
    public partial class S_DOC_File
    {
        ...
        public IControl GetDataGrid(bool needBorrowColumn = false, bool needStateColumn = false, int stateColumnwidth = 80)
        {
            ...
            foreach (var item in detailList)
            {
                ...
                column.Field = nodeAttr.InputType.IndexOf(ControlType.ButtonEdit.ToString()) >= 0 ? item.AttrField + "Name" : item.AttrField;
...
S_FileInfo.FullNodeID,S_FileInfo.CreateTime,S_FileInfo.State,S_FileInfo.BorrowState,";
            var attrList = this.S_DOC_FileAttr.ToList();
            foreach (var item in attrList)
            {
                var field = item.InputType.IndexOf(ControlType.ButtonEdit.ToString()) >= 0 ? item.FileAttrField + "Name" : item.FileAttrField;
                if (result.IndexOf("S_FileInfo." + field) >= 0)
                    continue;
                result += "S_FileInfo." + field + ",";
            }
