...
{
    public partial class S_R_Reorganize
    {
        public void Execute()
        {
            var user = FormulaHelper.GetUserInfo();
            if (!this.ReorganizeState.ToString().Equals(PhysicalReorganizeState.Execute.ToString()))
            {
                this.ReorganizeState = DocSystem.Logic.ReorganizeState.Execute.ToString();
                this.ReorganizeDate = DateTime.Now;
                this.ReorganizeUser = user.UserID;
                this.ReorganizeUserName = user.UserName;
            }
            else if (this.ReorganizeState == DocSystem.Logic.ReorganizeState.Execute.ToString())
            {
        ...
        public void Finish(bool check=false)
        {
            if (check)
                CheckFinish();
            var user = FormulaHelper.GetUserInfo();
            this.ReorganizeState = DocSystem.Logic.ReorganizeState.Finish.ToString();
            this.ReorganizeDate = DateTime.Now;
            this.ReorganizeUser = user.UserID;
            this.ReorganizeUserName = user.UserName;
        }
        ...
        {
            var user = FormulaHelper.GetUserInfo();
            this.ReorganizeState = DocSystem.Logic.ReorganizeState.Create.ToString();
            this.ReorganizeDate = DateTime.Now;
            this.ReorganizeUser = user.UserID;
            this.ReorganizeUserName = user.UserName;
        }
        ...
        {
            var user = FormulaHelper.GetUserInfo();
            if (this.ReorganizeState == DocSystem.Logic.ReorganizeState.Execute.ToString() && this.ReorganizeUser != user.UserID)
                throw new Formula.Exceptions.BusinessException("【" + this.TaskName + "】已经由【" + this.ReorganizeUserName + "】整编，请等【" + this.ReorganizeUserName + "】整编完成以后再操作");
        }
