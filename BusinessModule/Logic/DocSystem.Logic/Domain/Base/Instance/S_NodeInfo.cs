...
namespace DocSystem.Logic.Domain
{
    ...
    public partial class S_NodeInfo : S_BaseEntity
    {
        ...
        public string FullPathName
        {
            get
            {
                return string.IsNullOrEmpty(Convert.ToString(this.DataEntity["FullPathName"])) ? "" : this.DataEntity["FullPathName"].ToString();
            }
            set
            {
                this.DataEntity["FullPathName"] = value;
            }
        }
        ...
        public string ParentID
        {
            get
            {
                return this.DataEntity["ParentID"].ToString();
            }
            set
            {
                this.DataEntity["ParentID"] = value;
            }
        }
        #endregion
        ...
        public string CreateCarName()
        {
            ...
            if (this.ConfigInfo.ExtentionObject.GetValue("Ext_Car_UseFullName").ToLower() == "true")
            {
                ...
                fullName = string.Join(splitStr, this.Seniorities.Select(a => a.Name).ToList());
        ...
        public void ClearFiles()
        {
            ...
            sql = @"update S_R_Reorganize_DocumentList set ReorganizePath='',ReorganizeFullID='',ArchiveFileID='',ArchiveFileAttrs='' 
            where ArchiveFileID in (select ID from " + this.InstanceDB.DbName + "..S_FileInfo where NodeID='" + this.ID + "')";
            ...
            //实物整编下的文件
            sql = @"update S_R_PhysicalReorganize_FileDetail set ReorganizePath='',ReorganizeFullID='',ArchiveFileID='' 
            where ArchiveFileID in (select ID from " + this.InstanceDB.DbName + "..S_FileInfo where NodeID='" + this.ID + "')";
            configDB.ExecuteNonQuery(sql);
        }

        public void UpdateAllChildrenFullPathName()
        {
            StringBuilder sb = new StringBuilder();

            if (this.IsRoot)
                this.FullPathName = this.Name;
            else
                this.FullPathName = this.Parent.FullPathName + "$.$" + this.Name;
            sb.AppendLine(CreateUpdateFullPathNameSQL());

            foreach (var item in this.Children)
                item.CreateUpdateFullPathNameSQL(sb, this.FullPathName);
        ...
        public void CreateUpdateFullPathNameSQL(StringBuilder sb, string parentFullPathName)
        {
            this.FullPathName = parentFullPathName + "$.$" + this.Name;

            sb.AppendLine(this.CreateUpdateFullPathNameSQL());
        ...
        public string CreateUpdateFullPathNameSQL()
        {
            //this.FullPathName = this.Parent.FullPathName + "$.$" + this.Name;
            //this.DataEntity["FullPathName"] = this.FullPathName;
            var sql = @"update S_NodeInfo set FullPathName='{1}' where ID='{0}';
                                       update S_FileInfo set FullNodeName='{1}' where NodeID='{0}'; ";
            sql = string.Format(sql, this.ID, this.FullPathName, this.FullPathName);
            return sql;
        }
        ...
        public virtual void MoveTo(string nodeID)
        {
            ...
            if (node == null)
                throw new Formula.Exceptions.BusinessException("未能找到ID为【" + nodeID + "】的编目信息，移动卷册失败");
            if (this.IsNewModel) throw new Formula.Exceptions.BusinessException("新创建的卷册对象，不能使用Move方法进行卷册移动");
            this.FullPathID = node.FullPathID + "." + this.ID;
            this.FullPathName = node.FullPathName + "$.$" + this.Name;
        ...
        public void AddChild(S_NodeInfo node, bool withDefaultAttr = false)
        {
            ...
            node.DataEntity["FullPathName"] = this.FullPathName + "$.$" + node.Name;
            node.DataEntity["SpaceID"] = this.Space.ID;
            node.DataEntity["State"] = DocState.Normal;
            if (node.DataEntity.ContainsKey("SortIndex") && Tool.IsNullOrEmpty(node.DataEntity["SortIndex"]))
            {
                var maxIndex = 0;
                if (this.Children.Count > 0)
        ...
        public void FillConfigDefaultAttr()
        {
            ...
            foreach (var attr in attrs)
            {
                if (attr.DefaultValue.Split(',').Length > 0 && attr.InputType.IndexOf(ControlType.ButtonEdit.ToString()) >= 0 &&
                    this.DataEntity.ContainsKey(attr.AttrField + "Name"))
                {
                    SetDefualtValue(attr.AttrField, attr.DefaultValue.Split(',')[0]);
                    SetDefualtValue(attr.AttrField + "Name", attr.DefaultValue.Split(',')[1]);
                }
                else
                    SetDefualtValue(attr.AttrField, attr.DefaultValue);
            }
        }

        private void SetDefualtValue(string attrAttrField, string attrDefaultValue)
        {
            ...
            else
            {
                if (attrDefaultValue.IndexOf("{") >= 0)
                {
                    ...
                    if (defaultStr == "Now")
                        ...
                        this.DataEntity[attrAttrField] = CurrentUserInfo.UserID;
                        ...
                        if (defaultValue.Length > 1)
                        {
                            ...
                            if (nodePositon == "Root")
                            {
                                if (this.RootNode.DataEntity.ContainsKey(defaultField))
                                    this.DataEntity[attrAttrField] = this.RootNode.DataEntity[defaultField];
                            }
                            else
                            {
                                S_NodeInfo node = _getParentNode(this.Parent, nodePositon.Split('.').Length);
                                if (node.DataEntity.ContainsKey(defaultField))
                                    this.DataEntity[attrAttrField] = node.DataEntity[defaultField];
                            }
                        }
                        else
                        {
                            ...
                            if (this.DataEntity.ContainsKey(defaultField))
                                this.DataEntity[attrAttrField] = this.DataEntity[defaultField];
                        }
                    }
                }
            }
  ...
  where id=	'{0}'";
            this.InstanceDB.ExecuteNonQuery(string.Format(sql, this.ID));
        }
    }
}
