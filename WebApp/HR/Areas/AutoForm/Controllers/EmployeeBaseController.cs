...
namespace HR.Areas.AutoForm.Controllers
{
    public class EmployeeBaseController : HRFormContorllor<T_Employee>
    {
        ...
        {
            List<object> list = CommonMethod.GetSubEnum("HR.EmployeeSmallType", bigType);
...
{0}
        ...
        {
            var data = BusinessEntities.Set<T_EmployeeJob>().Where(c => c.EmployeeID == employeeID && c.IsDeleted != "1").WhereToGridData(qb);
            return Json(data);
        }
