...
where UserID='{0}'", userID, LoanAccountType.借款.ToString(), LoanAccountType.还款.ToString());
        ...
        protected override void BeforeSave(Dictionary<string, string> dic, Base.Logic.Domain.S_UI_Form formInfo, bool isNew)
        {
            if (!isNew)
            {
                var sql = string.Format(@"select SynID from {0} where ID='{1}' ", "S_EP_UserRefund", dic.GetValue("ID"));
        ...
        public void CancelReturnAmount(string rowIDs)
        {
            ...
            {
                var sql = string.Format(@"select SynID from {0} where ID='{1}' ", "S_EP_UserRefund", id);
                var db = SQLHelper.CreateSqlHelper(ConnEnum.DataInterface);
                var obj = db.ExecuteScalar(sql);
                if (obj != null && !string.IsNullOrEmpty(obj.ToString()))
                {
