...
namespace Base.Areas.Auth.Controllers
{
    public class UserController : BaseController<S_A_User>
    {
        ...
        public override ActionResult Edit()
        {
            ...
            if (authLevel == null || string.IsNullOrEmpty(authLevel.CorpID))
            {
                ViewBag.enumCompony = "null";
            }
            else
            {
                string sql = string.Format("select * from S_A_Org where ID in('{0}')", authLevel.CorpID.Replace(",", "','"));

                var dt = sqlHelper.ExecuteDataTable(sql);
                var em = dt.AsEnumerable().Select(c => new { value = c["ID"].ToString(), text = c["Name"].ToString() });
                ViewBag.enumCompony = JsonHelper.ToJson(em);
            }
            return View();
        }
