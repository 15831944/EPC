    ...
    public class CooperationExecuteController : ProjectFormContorllor<T_EXE_MajorPutInfo>
    {
        public override ActionResult PageView()
        {
            ...
            if (string.IsNullOrEmpty(wbsID))
            {
                ...
                var majorNode = projectInfo.S_W_WBS.FirstOrDefault(a => a.WBSValue == wbsValue);
                if (majorNode != null)
                    majorWBSID = majorNode.ID;
            }
            else
            {
                //无子项模式：计划内：wbsNode为提资包，计划外提资：wbsNode为root
                //有子项计划内提资：wbsNode为提资包
                var wbsNode = this.GetEntityByID<S_W_WBS>(wbsID.Split(',').FirstOrDefault());
                if (wbsNode.WBSType == WBSNodeType.CooperationPackage.ToString())
                {
        ...
        {
            if (subTableName == "FetchDrawingInfo")
            {
                if (detail.GetValue("DrawingID") == detail.GetValue("ShareFileID"))
                    detail.SetValue("DrawingID", FormulaHelper.CreateGuid());
                if (string.IsNullOrEmpty(detail.GetValue("ShareFileID")))
                    detail.SetValue("ShareFileID", FormulaHelper.CreateGuid());
            }
        }
