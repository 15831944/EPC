...
namespace Project.Areas.AutoUI.Controllers
{
    public class DigitalPublishingPublishApplyController : ProjectFormContorllor<T_EXE_PublishApply>
    {
        ...
        protected override void AfterGetData(DataTable dt, bool isNew, string upperVersionID)
        {
            ...
            {
                var projectInfo = this.GetEntityByID<S_I_ProjectInfo>(GetQueryString("ProjectInfoID"));
...
and p.Version = S_E_ProductVersion.Version) as pv where p.BatchID='" + batchID + "'");
                ...
                foreach (var item in list)
                {
                    ...
                    item.SetValue("ID", "");
                    if (item.GetValue("AuditState") == "Pass")
                        item.SetValue("CanSetUser", "0");
                    else
                        item.SetValue("CanSetUser", "1");
                    item.SetValue("ProjectManager", projectInfo == null ? "" : projectInfo.ChargeUserID);
                    item.SetValue("ProjectManagerName", projectInfo == null ? "" : projectInfo.ChargeUserName);
                    if (!string.IsNullOrEmpty(item.GetValue("DrawingCode")))
                        item.SetValue("ProductCode", item.GetValue("DrawingCode"));
                }
