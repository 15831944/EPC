...
namespace Project.Areas.AutoUI.Controllers
{
    public class MettingSignController : ProjectFormContorllor<T_EXE_MettingSign>
    {
        ...
        public override JsonResult DoBack(string taskExecID, string routingID, string execComment)
        {
            ...
            if (insDefStep.Code == "SignRevise")
            {
                var id = GetQueryString("ID");
                var entity = this.BusinessEntities.Set<T_EXE_MettingSign>().FirstOrDefault(a => a.ID == id);
                var products = this.BusinessEntities.Set<S_E_Product>().Where(a => id.Contains(a.CounterSignAuditID)).ToList();
                foreach (var item in products)
                {
                    item.CoSignState = CoSignState.SignRevise.ToString();
                    item.UpdateVersison();
                }
