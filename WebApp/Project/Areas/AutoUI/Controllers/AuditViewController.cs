...
and ID not in (select RelateID from S_N_Notice where RelateType='T_EXE_Audit_AdviceDetail' and ProjectInfoID='{1}')";
                    ...
                    #region 保存评论意见
                    foreach (var advice in entity.T_EXE_Audit_AdviceDetail.ToList())
                    {
                        var allList = JsonHelper.ToList(advice.CommentInfoJson);
                        if (string.IsNullOrEmpty(advice.CommentTempJson)) continue;
                        var commentList = JsonHelper.ToList(advice.CommentTempJson);
                        foreach (var comment in commentList)
                            allList.Add(comment);
                        advice.CommentTempJson = string.Empty;
                        advice.CommentInfoJson = JsonHelper.ToJson(allList);
                    }
                    #endregion
                }
            }
...
where m.AuditID='{0}' and _d.ID=m.ID";
        ...
        public JsonResult SyncMyAdvice(string AuditStep, string Type, string Advice, string WBSID)
        {
            ...
            if (wbs == null) throw new Formula.Exceptions.BusinessException("找不到指定的WBS");
