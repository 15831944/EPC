...
namespace Project.Areas.ProjectGroup.Controllers
{
    public class MainController : ProjectController
    {
        ...
        public ActionResult MainPage()
        {
            ...
            var executeState = ProjectCommoneState.Execute.ToString();
            var projectList = this.entities.Set<S_I_ProjectInfo>().Where(d => d.State == planState || d.State == executeState);
            var businessType = EnumBaseHelper.GetEnumDef("Market.BusinessType").EnumItem;
...
from dbo.S_C_ManageContract where IsSigned='{0}'";
            var dt = db.ExecuteDataTable(String.Format(sql, Market.Logic.ContractIsSigned.Signed.ToString()));
...
where S_I_ProjectInfo.State = 'Plan' or S_I_ProjectInfo.State = 'Execute'");
            ...
            string sql = @"select Sum(dept.RelationValue) as DataValue,Dept as ChargeDeptID,DeptName as ChargeDeptName
...
where BelongYear={0} group by Dept,DeptName";
            var receiptDt = db.ExecuteDataTable(String.Format(sql, belongYear));
            ...
            var costDt = db.ExecuteDataTable(String.Format(sql, belongYear));
...
where MileStoneState='{0}' group by ChargerDept,ChargerDeptName";
            var canReceiptDt = db.ExecuteDataTable(String.Format(sql, true.ToString()));

            sql = @"select isnull(SUM(DataValue),0) DataValue,ChargeDeptID,ChargeDeptName from (
select (isnull(DeptValue,0)-isnull(SumDeptReceiptValue,0)) as DataValue,
...
group by ChargeDeptID,ChargeDeptName";
            var contractDt = db.ExecuteDataTable(String.Format(sql, belongYear,Market.Logic.ContractIsSigned.Signed.ToString()));
...
group by DeptID,DeptName,MistakeLevel";
            ...
            var mistakeDt = this.SqlHelper.ExecuteDataTable(String.Format(sql, belongYear));
            foreach (var item in result)
            {
                var deptID = item.GetValue("DeptID");
                var mistakeSum = 0m;
        ...
        private List<Dictionary<string, object>> _getResourceLis(string belongYear)
        {
            ...
            string sql = @"select Sum(dept.RelationValue) as DataValue,Dept as ChargerDeptID,DeptName as ChargerDeptName
 from S_C_Receipt_DeptRelation dept left join S_C_Receipt receipt on dept.S_C_Receipt_ID = receipt.ID
where BelongYear={0} group by Dept,DeptName";
...
group by UserDeptID,UserDeptName,BelongYear,BelongMonth,BelongQuarter", belongYear, "Production"));
...
where IsSigned='{0}' group by Dept,DeptName";
            var remainContractDt = marketDb.ExecuteDataTable(String.Format(sql, Market.Logic.ContractIsSigned.Signed.ToString()));
...
and IndicatorType='YearIndicator'";
            ...
            sql = "select isnull(Sum(RelationValue),0) as DataValue from S_C_Receipt_DeptRelation dept left join S_C_Receipt receipt on dept.S_C_Receipt_ID = receipt.ID where BelongYear='{0}' and Dept='{1}'";
            receiptValue = Math.Round(Convert.ToDecimal(db.ExecuteScalar(String.Format(sql, DateTime.Now.Year, deptID))) / 10000);
            sql = @"select isnull(SUM(DataValue),0) DataValue from (
select isnull(DeptValue,0) as DataValue from S_C_ManageContract_DeptRelation dept
left join S_C_ManageContract con on dept.S_C_ManageContractID = con.ID 
where  BelongYear='{0}' and Dept='{1}'
union 
select isnull(DeptValue,0) as DataValue from  S_C_ManageContract_Supplementary_DeptRelation dept
left join S_C_ManageContract_Supplementary ad on ad.ID=dept.S_C_ManageContract_SupplementaryID 
where  ad.BelongYear='{0}' and Dept='{1}'
) tb";
        ...
        {
            string sql = @"select Sum(dept.RelationValue) as DataValue,Dept as ChargerDeptID,DeptName as ChargerDeptName
,BelongYear,BelongMonth from S_C_Receipt_DeptRelation dept left join S_C_Receipt receipt on dept.S_C_Receipt_ID = receipt.ID
where Dept='{0}' and BelongYear={1} group by Dept,DeptName,BelongYear,BelongMonth";
            var db = SQLHelper.CreateSqlHelper(ConnEnum.Market);
...
where ProjectDeptID='{0}' and BelongYear={1} group by ProjectDeptID,ProjectDeptName,BelongYear,BelongMonth";
        ...
        public JsonResult GetContractInfo(string lastYear, string DeptID, string AnalysisType)
        {
            ...
            string sql = @"select isnull(SUM(DataValue),0) Value,BelongYear,BelongMonth from (
select isnull(DeptValue,0) as DataValue,
Dept as ChargeDeptID,DeptName as ChargeDeptName,
BelongYear,BelongQuarter,BelongMonth from S_C_ManageContract_DeptRelation dept
left join S_C_ManageContract con on dept.S_C_ManageContractID = con.ID 
where IsSigned='{1}' and Dept='{0}' and {2}
union 
select isnull(DeptValue,0) as DataValue,
Dept as ChargeDeptID,DeptName as ChargeDeptName,
ad.BelongYear,ad.BelongQuarter,ad.BelongMonth from  S_C_ManageContract_Supplementary_DeptRelation dept
left join S_C_ManageContract_Supplementary ad on ad.ID=dept.S_C_ManageContract_SupplementaryID 
where ad.SignDate is not null and ad.SignDate !='' and Dept='{0}' and {2}
) tb
group by BelongYear,BelongMonth";
            sql = String.Format(sql, DeptID,Market.Logic.ContractIsSigned.Signed.ToString(), " BelongYear>='" + startYear + "' and BelongYear<='" + endYear + "' ");
...
left join (select Sum(RelationValue) as ProjectReceiptValue,ProjectID 
from S_C_Receipt_ProjectRelation group by ProjectID) ProjectReceiptInfo
on S_I_Project.ID=ProjectReceiptInfo.ProjectID
