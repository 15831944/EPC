...
{
    public class ProductionSubsidyController : ExpensesFormController<S_EP_ProductionSubsidyApply>
    {
        public JsonResult ValidateSubsidy(string cbsNodeID)
        {
            ...
            var adjustRTCount = this.SQLDB.ExecuteScalar(string.Format("select count(*) from S_EP_ProductionSubsidyApply where FlowPhase != 'End' and CBSNode = '{0}'", cbsNodeID));
        ...
        protected override void OnFlowEnd(S_EP_ProductionSubsidyApply data, Workflow.Logic.Domain.S_WF_InsTaskExec taskExec, Workflow.Logic.Domain.S_WF_InsDefRouting routing)
        {
            if (data != null)
            {
                data.Push();
