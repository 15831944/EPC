...
{
    public class DocumentSealUpApplyController :  DocSystemFormContorllor<S_SU_DocumentSealUp>
    {
        protected override void BeforeSave(Dictionary<string, string> dic, Base.Logic.Domain.S_UI_Form formInfo, bool isNew)
        {
            ...
            if (!string.IsNullOrEmpty(detailJsonStr))
            {
                ...
                foreach (var spaceID in spaceIDs)
                {
                    ...
                    //获取全部子表项ID
                    var oldIds = this.DocConstSQLDB.ExecuteDataTable(string.Format("select RelateID from S_SU_DocumentSealUp_DetailList where S_SU_DocumentSealUpID='{0}' and SpaceID='{1}'", formID, spaceID))
                    ...
                    {
                        sb.AppendLine("update S_NodeInfo set State = '" + DocState.Normal.ToString() + "' where State ='" + DocState.SealUpApply.ToString() + "' and FullPathID like '%" + nodeid + "%'");
                        sb.AppendLine("update S_FileInfo set State = '" + DocState.Normal.ToString() + "' where  State ='" + DocState.SealUpApply.ToString() + "' and FullNodeID like '%" + nodeid + "%'");
                    }
                    if (sb.Length > 0)
        ...
        {
            var ids = Request["ID"];
            var entity = this.BusinessEntities.Set<S_SU_DocumentSealUp>().SingleOrDefault(c => c.ID == ids);
            if (entity != null)
            {
                var detailIst = entity.S_SU_DocumentSealUp_DetailList.ToList();
                var spaceList = new List<S_DOC_Space>();
                ...
                foreach (var spaceID in spaceIDs)
                {
                    ...
                    foreach (var nodeid in nodeIds)
                    {
                        //将封存申请中、封存状态的节点及文件设置为禁用
                        sb.AppendLine("update S_NodeInfo set State = '" + DocState.Normal.ToString() + "' where State in ('" + DocState.SealUpApply.ToString() + "','" + DocState.SealUp.ToString() + "') and FullPathID like '%" + nodeid + "%'");
                        sb.AppendLine("update S_FileInfo set State = '" + DocState.Normal.ToString() + "' where State in ('" + DocState.SealUpApply.ToString() + "','" + DocState.SealUp.ToString() + "') and FullNodeID like '%" + nodeid + "%'");
                    }
                    if (sb.Length > 0)
                        db.ExecuteNonQuery(sb.ToString());
                }
        ...
        protected override void OnFlowEnd(S_SU_DocumentSealUp entity, S_WF_InsTaskExec taskExec, S_WF_InsDefRouting routing)
        {
            var detailIst = entity.S_SU_DocumentSealUp_DetailList.ToList();
            var spaceList = new List<S_DOC_Space>();
            var spaceIDs = detailIst.Where(a => !string.IsNullOrEmpty(a.SpaceID)).Select(a => a.SpaceID).ToList();
            foreach (var spaceID in spaceIDs)
            {
                ...
                foreach (var nodeid in nodeIds)
                {
                    sb.AppendLine("update S_NodeInfo set State = '" + DocState.SealUp.ToString() + "',RackNumber='" + entity.SealUpRackNumber + "',RackNumberName='" + entity.SealUpRackNumberName + "',StorageRoom='" + entity.SealUpStorageRoom + "',StorageRoomName='" + entity.SealUpStorageRoomName+ "' where FullPathID like '%" + nodeid + "%'");
                    sb.AppendLine("update S_FileInfo set State = '" + DocState.SealUp.ToString() + "' where FullNodeID like '%" + nodeid + "%'");
                }
                if (sb.Length > 0)
                    db.ExecuteNonQuery(sb.ToString());
            }
            foreach (var detail in detailIst)
            {
                if (detail.RelateType.Equals("Node"))
                {
                    ...
                    InventoryFO.CreateNewInventoryLedger(node, "\"库房\"修改为：\"" + entity.SealUpStorageRoomName + "\";\"货架号\"修改为：\"" + entity.SealUpRackNumberName + "\"", InventoryType.SealUp);
                }
                else {
                    ...
                    InventoryFO.CreateNewInventoryLedger(file, "\"库房\"修改为：\"" + entity.SealUpStorageRoomName + "\";\"货架号\"修改为：\"" + entity.SealUpRackNumberName + "\"", InventoryType.SealUp);
                }
            }
        }
    }
